...
Waiting: the `wait_for` states are documented in `reference/waits` (`llm_browser.docs`).
...
language, and `reference/extract` for typed extraction (pydantic models, YAML-declared
...
| `nodriver` | all writes go through trusted `Input.dispatch*` CDP events | Chromium via raw CDP; a few reads use JS (see `llm_browser.drivers`) |
...
`src/llm_browser/drivers/__init__.py`. Build-vs-buy investigation of the 2026
...
Details: the `BrowserSession` docstring, or `reference/session`.
