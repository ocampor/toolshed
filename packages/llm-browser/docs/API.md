...
| `wait_for_value(selector, value, exact=False, timeout=, interval=)` | Poll until the field holds `value` (`value`, or `innerText` on a contenteditable) — substring unless `exact`; the `TimeoutError` names what it held last |
