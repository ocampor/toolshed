...
| `click(selector, dispatch=False, humanize=None, behavior=None)` | Wait for the element, then click it — humanized mouse path when `Behavior.mouse_move`, which wheels the target into view, then re-checks what the pointer landed on and raises `ValueError` (`covered-after-move`, or `hit-test-failed` when the page could not be asked) rather than click something the path opened, answering the `HitTarget` (`tag`, `text`, `class_name`) it did click. `humanize=True`/`False` forces that path on or off for this call, `behavior=` runs it under a `Behavior` of your own. `dispatch=True` fires an untrusted DOM `click` event instead, for overlays real input cannot reach, on an element that need only be attached |
| `fill(selector, value, verify="changed", humanize=None, behavior=None)` | Set a field's value — cleared with select-all + Delete and typed character by character when `Behavior.fill_as_type`, otherwise a single `fill` (zero key events). `ValueError` naming expected and actual (lengths only, for a password field) when the field ends where it was right before the value went in, or empty (`verify="exact"`: on any mismatch) |
...
| `set_checked(selector, checked, dispatch=False, behavior=None)` | Check or uncheck a checkbox. `dispatch=True` clicks a hidden (attached) box with an untrusted click when its state differs, and raises `ValueError` on a disabled box or one that still differs |
...
| `wait_for_value(selector, value, exact=False, timeout=, interval=)` | Poll until the field holds `value` (`value`, or `innerText` on a contenteditable) — substring unless `exact`; `MatchCountError` when the count breaks the step's match rule (one match, since `wait_for` states none); the `TimeoutError` names what it held last (its length, for a password field) |
