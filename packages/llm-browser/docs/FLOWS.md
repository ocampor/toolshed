...
| Action | Required | Optional | Notes |
|---|---|---|---|
| `read` | — | `extract` (see [below](#extract-spec-for-read-action)), `path` | Extract structured data as dicts. No `extract:` reads each match's own text as `text` — `read` on `body` gives `[{ text: … }]` |
...
Leaving `extract` out entirely is that last form under the name `text`: one
`{ text: … }` per matched element.
