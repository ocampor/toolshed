...
The reads answer about the DOM as it is right now: a miss is 0 / False / a
locator that matches nothing, never a retry inside the driver and never a
raise. `llm_browser.waits` owns the deadline, and a read that blocked or
...
def playwright_case() -> Iterator[DriverCase]:
    ...
    yield DriverCase(
        ...
        lambda: locator.wait_for.mock_calls + locator.first.wait_for.mock_calls,
    )
...
def test_count_never_reaches_a_retrying_api(case: DriverCase) -> None:
